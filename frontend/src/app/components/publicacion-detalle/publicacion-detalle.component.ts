...
export class PublicacionDetalleComponent implements OnInit {
  ...
  const usuarioIdString = String(this.usuarioActualId);
  const resultado = this.publicacion.meGusta?.some(id => String(id) === usuarioIdString) || false;
  
  console.log('🔍 Verificando me gusta en detalle:', {
    usuarioId: usuarioIdString,
    meGusta: this.publicacion.meGusta,
...
toggleMeGusta(): void {
  console.log('💗 Toggle me gusta');
  console.log('❤️ Ya dio me gusta:', this.yaDioMeGusta());
...
darMeGusta(): void {
  ...
  this.publicationsService.darMeGusta(this.publicacionId).subscribe({
    next: (pubActualizada: any) => {
      console.log('✅ Me gusta agregado:', pubActualizada.meGusta);
      if (this.publicacion) {
...
quitarMeGusta(): void {
  ...
  console.log('💔 Quitando me gusta');
  
  this.publicationsService.quitarMeGusta(this.publicacionId).subscribe({
    next: (pubActualizada: any) => {
      console.log('✅ Me gusta quitado:', pubActualizada.meGusta);
      if (this.publicacion) {
        this.publicacion.meGusta = pubActualizada.meGusta || [];
      }
    },
